...
func parseAttribute(decl *parser.Decl) (attr agnostic.Attribute, err error) {
		var name, typeName string

	// Attribute name
	if decl.Token != parser.StringToken {
		return agnostic.Attribute{}, fmt.Errorf("engine: expected attribute name, got %v", decl.Token)
	}
	...
	if decl.Decl[0].Token != parser.StringToken {
		return agnostic.Attribute{}, fmt.Errorf("engine: expected attribute type, got %v:%v", decl.Decl[0].Token, decl.Decl[0].Lexeme)
	}
